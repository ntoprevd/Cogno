...
- 侧边栏手势仍是敏感区域，修改前要完整阅读 `SidebarDrawer` 相关实现。
- 欢迎页标识和流式文案只是第一版，用户还可能继续微调图形、句子、字体、间距。
- 长按菜单、弹窗、Toast 已完成一轮收敛，但还不是最终设计系统。
- assistant 下方工具栏需要真机观察是否过密、是否影响长文本阅读。
- Markdown 表格是轻量实现，不是完整 CommonMark 表格解析。

### P1：设置页和语言体系

- 设置页支持语言偏好，但全站文案尚未完整迁移到 Android string resources。
...
6. 不要一次性启动主题笔记、账户系统、文件解析、完整多语言资源化。它们都重要，但应分阶段做。
